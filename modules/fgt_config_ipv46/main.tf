...
data "template_file" "fgt" {
  ...
  vars = {
    ...
    config_route6    = var.static_route_cidrs6 != null ? data.template_file.config_route6.rendered : ""
...
data "template_file" "config_basic" {
  ...
  vars = {
    ...
    gw_ipv6      = lookup(local.port1_config, "gw_ipv6", "")
  }
}
...
data "template_file" "config_interfaces" {
  ...
  vars = {
    ...
    ipv6      = local.ports_config[count.index]["ipv6"]
    mask      = local.ports_config[count.index]["mask"]
    mask_ipv6 = local.ports_config[count.index]["mask_ipv6"]
...
data "template_file" "config_route6" {
  template = templatefile("${path.module}/templates/fgt_static6.conf", {
    route_cidrs = var.static_route_cidrs6
    port        = local.map_type_port["private"]
    gw          = local.map_type_gw6["private"]
  })
}
