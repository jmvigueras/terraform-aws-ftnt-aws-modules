locals {
  ...
  # Create subnet map of maps [{subnet_name, cidr, az, subnet_num}]
  subnets_map = merge(
  ...
  o_subnets = { for i, az in var.azs :
    "az${i + 1}" => { for subnet in local.subnets_list :
      subnet => {
        ...
        cidr_ipv6 = var.enable_ipv6 ? lookup(aws_subnet.subnets, "${subnet}-az${i + 1}", { ipv6_cidr_block = "" }).ipv6_cidr_block : ""
  ...
  o_subnet_list = [for i, subnet in local.subnets_name :
    { name      = subnet
      ...
      cidr      = cidrsubnet(var.cidr, ceil(log(local.subnets_count, 2)), i)
      cidr_ipv6 = var.enable_ipv6 ? lookup(aws_subnet.subnets, subnet, { ipv6_cidr_block = "" }).ipv6_cidr_block : ""
  ...
  # Create map of maps with subnets IPv6 cidrs
  o_subnet_cidrs_ipv6 = { for i, az in var.azs :
    "az${i + 1}" => { for subnet in local.subnets_list :
      subnet => lookup(aws_subnet.subnets, "${subnet}-az${i + 1}", { ipv6_cidr_block = "" }).ipv6_cidr_block
    }
  }
  # Create map of maps with public subnets IPv6 cidrs
  o_subnet_public_cidrs_ipv6 = { for i, az in var.azs :
    "az${i + 1}" => { for subnet in local.subnets_public :
      subnet => lookup(aws_subnet.subnets, "${subnet}-az${i + 1}", { ipv6_cidr_block = "" }).ipv6_cidr_block
    }
  }
  # Create map of maps with private subnets IPv6 cidrs
  o_subnet_private_cidrs_ipv6 = { for i, az in var.azs :
    "az${i + 1}" => { for subnet in local.subnets_private :
      subnet => lookup(aws_subnet.subnets, "${subnet}-az${i + 1}", { ipv6_cidr_block = "" }).ipv6_cidr_block
    }
  }
