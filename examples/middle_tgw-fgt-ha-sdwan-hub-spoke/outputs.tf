...
output "hub_mgmt" {
  value = {
    fgt1_mgmt   = "https://${one(module.hub_nis.fgt_ni_list["az1.fgt1"]["mgmt_eips"])}:${local.admin_port}"
    fgt2_mgmt   = "https://${one(module.hub_nis.fgt_ni_list["az2.fgt1"]["mgmt_eips"])}:${local.admin_port}"
    fgt_user    = "admin"
...
output "sdwan_mgmt" {
  value = { for k, v in module.sdwan :
    "sdwan${k + 1}" =>
    {
      fgt1_mgmt   = "https://${one(module.sdwan_nis[k].fgt_ni_list["az1.fgt1"]["public_eips"])}:${local.admin_port}"
      fgt_user    = "admin"
